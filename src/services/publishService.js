...
function buildReadme(articleContent, date) {
  return `# AI & Tech Daily
...
async function publishToPublicRepo(articleContent, date) {
  ...
  try {
    ...
    const readmeContent = buildReadme(articleContent, date);
    await fs.writeFile(path.join(tmpDir, 'README.md'), readmeContent, 'utf-8');

    const imagesDir = path.resolve(process.cwd(), 'images');
    if (await fs.pathExists(imagesDir)) {
      const destImages = path.join(tmpDir, 'images');
      await fs.copy(imagesDir, destImages, { overwrite: true });
